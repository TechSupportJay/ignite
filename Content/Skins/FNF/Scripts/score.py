def spawn_a_text(self, pos):
    ...
    score_text.set_property("font_size", 18)
...
def update(self, dt):
    for st in self.all_texts:
        st.set_property("text", f"Score: {player_stats["score"]}  |  Misses: {player_stats["misses"]}  |  Rating: {skin_texts['Gameplay']['ranks'][player_stats['rank']]} ({round(player_stats["accuracy"] * 100, 2)}%)")
...
def note_hit(self, a, b):
    ...
    i = 0
    for st in self.all_texts:
        st.set_property("scale", [1.1,1.1])
        camera.do_tween(f"score_text_bump_{i}_x", st, "scale:x", 1, 0.5, "cubic", "out")
        camera.do_tween(f"score_text_bump_{i}_y", st, "scale:y", 1, 0.5, "cubic", "out")
        i += 1